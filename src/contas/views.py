...
@login_required 
def contas(request):
    contas_a_pagar = ContaPagar.objects.filter(status=CONTA_STATUS_APAGAR, usuario=request.user)
    contas_a_receber = ContaReceber.objects.filter(status=CONTA_STATUS_APAGAR, usuario=request.user)
...
def contas_por_classe(request, classe, titulo):
    ...
    paginacao = Paginator(contas, 5)#paginação
